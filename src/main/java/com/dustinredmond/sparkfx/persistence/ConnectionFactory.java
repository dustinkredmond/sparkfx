...
/**
 * Factory class for getting a {@code java.sql.Connection}
 * to the default, or optionally, user-defined
 * SQLite database.
 */
public class ConnectionFactory {
    ...
    protected Connection connect() {
        ...
        try {
            if (!DB_USER_PATH.isEmpty()) {
                conn = DriverManager
                    .getConnection("jdbc:sqlite:" + DB_USER_PATH);
            } else {
            ...
            if (ServerContext.isHeadless()) {
                ...
                throw new RuntimeException("Unable to get local database"
                    + " connection.", e);
            }
        }
        return conn;
    ...
    /**
     * Default database URL for SQLite Connection.
     */
    private static final String DB_URL = "jdbc:sqlite:sparkfx.db";

    /**
     * Path to user-defined Database path. Empty String if not previously set.
     */
